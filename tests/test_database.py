...
class TestInitDatabase:
    def test_creates_iocs_table(self, tmp_db):
        ...
        cur = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='iocs'")
    ...
    def test_creates_feed_status_table(self, tmp_db):
        with patch("database.DB_PATH", tmp_db):
            init_database()
        conn = sqlite3.connect(tmp_db)
        cur = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='feed_status'")
        assert cur.fetchone() is not None
        conn.close()
    ...
    def test_creates_alerts_table(self, tmp_db):
        with patch("database.DB_PATH", tmp_db):
            init_database()
        conn = sqlite3.connect(tmp_db)
        cur = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='alerts'")
        assert cur.fetchone() is not None
        conn.close()
    ...
    def test_iocs_unique_constraint(self, tmp_db):
        ...
        conn.execute(
            "INSERT INTO iocs (type, value, threat_type, source, first_added, last_updated) VALUES (?, ?, ?, ?, ?, ?)",
